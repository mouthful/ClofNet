...
class EVFNnRB_GCL(E_GCL):
    ...
    def forward(self, h, edge_index, coord, coord_raw, vel, edge_attr=None, node_attr=None):
        row, col = edge_index
        # radial, coord_diff, coord_cross, coord_vertical = self.coord2radial(edge_index, coord)
        coord_diff, coord_cross, coord_vertical = self.coord2radial(edge_index, coord_raw)
...
class Envelope(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x_pow_p1 = x_pow_p0 * x
        x_pow_p2 = x_pow_p1 * x
        return 1. / (x+0.0001) + a * x_pow_p0 + b * x_pow_p1 + c * x_pow_p2

class dist_encoder(torch.nn.Module):
    def __init__(self, num_radial, cutoff=5.0, envelope_exponent=5):
        super(dist_encoder, self).__init__()
        self.cutoff = cutoff
        self.envelope = Envelope(envelope_exponent)

        freq = torch.arange(1, num_radial + 1) * PI
        self.freq = torch.nn.Parameter(torch.Tensor(freq))
