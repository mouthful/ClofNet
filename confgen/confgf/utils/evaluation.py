...
def get_rmsd_confusion_matrix(data: Data, useFF=False, reverse=False):
    if not reverse:
        pos_ref = data.pos_ref.view(-1, data.num_nodes, 3)
        pos_gen = data.pos_gen.view(-1, data.num_nodes, 3)
        num_gen = pos_gen.size(0)
        num_ref = pos_ref.size(0)
        assert num_gen == data.num_pos_gen.item()
        assert num_ref == data.num_pos_ref.item()
    else:
        pos_ref = data.pos_gen.view(-1, data.num_nodes, 3)
        pos_gen = data.pos_ref.view(-1, data.num_nodes, 3)
        num_gen = pos_gen.size(0)
        num_ref = pos_ref.size(0)
        assert num_gen == data.num_pos_ref.item()
        assert num_ref == data.num_pos_gen.item()

    rmsd_confusion_mat = -1 * np.ones([num_ref, num_gen],dtype=np.float)
    
    for i in range(num_gen):
        gen_mol = utils.set_rdmol_positions(data.rdmol, pos_gen[i])
        if useFF:
            #print('Applying FF on generated molecules...')
            MMFFOptimizeMolecule(gen_mol)
        for j in range(num_ref):
            ref_mol = utils.set_rdmol_positions(data.rdmol, pos_ref[j])
...
def evaluate_conf_prec(data: Data, useFF=False, threshold=0.5):
    rmsd_confusion_mat = get_rmsd_confusion_matrix(data, useFF=useFF, reverse=True)
    rmsd_ref_min = rmsd_confusion_mat.min(-1)
    return (rmsd_ref_min<=threshold).mean(), rmsd_ref_min.mean()

def evaluate_conf_extend(data: Data, useFF=False, threshold=0.5):
    rmsd_confusion_mat = get_rmsd_confusion_matrix(data, useFF=useFF)
    rmsd_ref_min = rmsd_confusion_mat.min(-1)
    rmsd_gen_min = rmsd_confusion_mat.min(0)
    recall_cov, recall_mat = (rmsd_ref_min<=threshold).mean(), rmsd_ref_min.mean()
    prec_cov, prec_mat = (rmsd_gen_min<=threshold).mean(), rmsd_gen_min.mean()
    return recall_cov, recall_mat, prec_cov, prec_mat
