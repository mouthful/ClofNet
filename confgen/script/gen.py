...
if __name__ == '__main__':
    ...
    parser.add_argument('--generator', type=str, help='type of generator [EquiGF, EquiPCGF]', required=True)
    ...
    if config.test.output_path is not None:
        config.test.output_path = os.path.join(config.test.output_path, config.train.Name)
    ...
    config.train.wandb.Enable = False
    ...
    init_checkpoint = os.path.join(config.test.init_checkpoint, config.train.Name)
    solver.load(init_checkpoint, epoch=args.eval_epoch)
    ...
    if args.start != -1 and args.end != -1:
        solver.generate_samples_from_testset(args.start, args.end, args.eval_epoch, \
